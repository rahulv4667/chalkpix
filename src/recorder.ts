import Konva from "https://unpkg.com/konva@8/konva.min.js";
import Peer from "https://unpkg.com/peerjs@1.3.1/dist/peerjs.min.js";
// import Picker from "https://unpkg.com/vanilla-picker@2";
...
let circle_counter: number = 0;
let rect_counter: number = 0;
let brush_counter: number = 0;
let text_counter: number = 0;
let recording_array: Array<any> = [];
let audioChunks: Array<any> = [];
let peerDataConnections: Array<any> = [];
let peerDataLastIndex: Array<any> = [];
let peerStreamConnections: Array<any> = [];
let myId: number | string = 0;
let isRecording: boolean = false;
let hasAudioPermission: boolean = false;
let audioRecorder: MediaRecorder | undefined = undefined;
let audioblob: Blob | undefined = undefined;

let canvasStream: any = null;
let mediaRecorder: any = null;
let recordedBlobs: any = null;
let sourceBuffer: SourceBuffer | undefined = undefined;

// handling source open for media source(video recording)
function handleVideoSourceOpen(event: Event) {
...
function handleVideoDataAvailable(event: BlobEvent) {
...
function handleVideoStop(event: BlobEvent) {
...
recording_array.push = function(...items: any[]) {
//   let len = Array.prototype.push.apply(this, arguments);
  let len = Array.prototype.push.apply(this, items);
...
const record_button: HTMLElement | null = document.getElementById("recordBtn");
...
function generate_audio_blob(event: BlobEvent | null) {
...
if(navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
  ...
  .then(function(stream) {
    ...
    audioRecorder.onstop = generate_audio_blob as EventListener;
...
peer.on('open', function(id) {
  myId=id;
  console.log('id=', id);
  (document.getElementById('myId') as HTMLElement).innerHTML = myId as string;
  ...
  peer.on('call', function(call) {
    ...
    console.log('stream being sent:', (audioRecorder as MediaRecorder).stream);
    call.answer((audioRecorder as MediaRecorder).stream);
...
layerBackgroundRect.on('mousedown touchstart click', function(e) {
    ...
    (menuNode as HTMLElement).style.display='none';
    layer.draw();
});
...
const brushBtn = document.getElementById('brush-btn') as HTMLButtonElement;
...
const textBtn = document.getElementById('text-btn') as HTMLButtonElement;
...
laserBtn.addEventListener('click', (e) => {
    ...
    stage.on('mousemove', function() {
        ...
        let pos = stage.getPointerPosition() as Konva.Vector2d;
        menuNode.style.display='none';
        laser.x(pos.x);
        laser.y(pos.y);
        laser.moveToTop();
...
function useBrush() {
    ...
    stage.on('mousemove touchmove', function(e) {
        if(!isPaint) return;
        pos = stage.getPointerPosition() as Konva.Vector2d;
...
function addText() {
    ...
    textNode.on('dblclick dbltap', function() {
                ...
                function removeTextarea() {
                  (textarea.parentNode as (Node&ParentNode)).removeChild(textarea);
...
let menuNode = document.getElementById('menu') as HTMLElement;
(document.getElementById('move-layer-up') as HTMLElement).addEventListener('click', () => {
  currentShape.moveUp();
...
(document.getElementById('move-layer-down') as HTMLElement).addEventListener('click', () => {
...
(document.getElementById('move-to-top') as HTMLElement).addEventListener('click', () => {
...
(document.getElementById('move-to-bottom') as HTMLElement).addEventListener('click', () => {
...
let fillpicker = new Picker(document.getElementById('fill-color') as HTMLElement);
...
let strokepicker = new Picker(document.getElementById('stroke-color') as HTMLElement);
...
(document.getElementById('delete-shape') as HTMLElement).addEventListener('click', () => {
  currentShape.destroy();
  selected_transformer.nodes([]);
  if(isRecording) {
    recording_array.push({
...
(document.getElementById('left-align') as HTMLElement).addEventListener('click', () => {
...
(document.getElementById('right-align') as HTMLElement).addEventListener('click', () => {
...
(document.getElementById('center-align') as HTMLElement).addEventListener('click', () => {
...
(document.getElementById('bold-text') as HTMLElement).addEventListener('click', () => {
...
(document.getElementById('italic-text') as HTMLElement).addEventListener('click', () => {
  let styles = currentShape.fontStyle().split(" ");
...
(document.getElementById('underline-text') as HTMLElement).addEventListener('click', () => {
...
(document.getElementById('strikethrough-text') as HTMLElement).addEventListener('click', () => {
  let styles = currentShape.textDecoration().split(" ");
...
stage.on('contextmenu', function (e) {
  ...
  currentShape = e.target;
  if(currentShape === layerBackgroundRect) return;
  (document.getElementById("row-alignment") as HTMLElement).style.display = 
      (currentShape instanceof Konva.Text)?'inline-block':'none';
  (document.getElementById("row-decoration") as HTMLElement).style.display = 
      (currentShape instanceof Konva.Text)?'inline-block':'none';
  ...
  menuNode.style.top =
    containerRect.top/*window.scrollY*/ + (stage.getPointerPosition() as Konva.Vector2d).y + 4 + 'px';
  menuNode.style.left =
    containerRect.left/*window.scrollX*/ + (stage.getPointerPosition() as Konva.Vector2d).x + 4 + 'px';
...
canvasStream = (layer.getCanvas()._canvas as CanvasElement).captureStream();
...
function generateBlob() {
  ...
  if(audioblob === null || audioblob === undefined) generate_audio_blob(null);
  console.log(audioblob);
  // let audio_string = await audioblob.text();
  // console.log(audio_string);
  const final_str = length+"\n"+json_string; 
  
  const blob = new Blob([final_str, audioblob as BlobPart], {type: 'text/plain'});
  const url = URL.createObjectURL(blob);
  
  const cpxbutton = document.getElementById('cpxDownload') as HTMLButtonElement;
  ...
  const videoButton = document.getElementById('cpxDownloadVideo') as HTMLButtonElement;
...
(record_button as HTMLElement).addEventListener('click', function(e) {
  ...
  if(!isRecording) {
    ...
    (audioRecorder as MediaRecorder).start(1000);
    console.log((audioRecorder as MediaRecorder).state);
    last_activity_timestamp=Date.now();
    recording_array.push({
      ts:0,
      type:"start"
    });
    (document.getElementById('startBtn') as HTMLElement).innerHTML = 'Stop Recording/Streaming';
    (document.getElementById('status') as HTMLElement).innerHTML = `Recording`;
  } else {
    recording_array.push({
      ts: Date.now() - last_activity_timestamp,
      type: "end"
    });
    (audioRecorder as MediaRecorder).requestData();
    (audioRecorder as MediaRecorder).stop();
    console.log((audioRecorder as MediaRecorder).state);
    ...
    // resetting
    (document.getElementById('status') as HTMLElement).innerHTML = `Idle`;
    isRecording=false;
    recording_array = [];
    let idx: any=0;
    (document.getElementById('startBtn') as HTMLElement).innerHTML = 'Stop Recording/Streaming';
    (record_button as HTMLButtonElement).disabled=true;
  }
});
