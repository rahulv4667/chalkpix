...
const player_div = document.getElementById('player') as HTMLElement;
let audioPlayer: any = null;
let player_array = new Array();
let start_time = 0;
let elapsed_time = 0;
let isPlaying = false;
let to_be_chalked: any = null;
let timer_timeout: any = null;
let idx = 0;
...
const stream_radio = document.getElementById('Stream') as HTMLInputElement;
const archive_radio = document.getElementById('Archive') as HTMLInputElement;
...
stream_radio.addEventListener('change', function() {
    ...
    if(stream_radio.checked) {
        ...
        // player_div.children=[];
        player_div.childNodes.forEach((element: Node) => {player_div.removeChild(element)});
        // const chldrn = player_div.children;
        // for(let chld of chldrn){
        //     player_div.removeChild(chld);
        // }
        player_div.appendChild(
            (document.getElementById('streamplayer') as HTMLTemplateElement).content.cloneNode(true));
    }
    start_time = 0;
    elapsed_time = 0;
    isPlaying=false;
    ...
    (document.getElementById('streamidsubmit') as HTMLElement).onclick = streamer;
});
...
archive_radio.addEventListener('change', function() {
    if(archive_radio.checked) {
        ...
        // player_div.childNodes=[];
        // player_div.children=[];
        player_div.childNodes.forEach((node: Node) => player_div.removeChild(node));
        // const chldrn = player_div.children;
        // for(let chld of chldrn){
        //     player_div.removeChild(chld);
        // }
        player_div.appendChild(
            (document.getElementById('archivalplayer') as HTMLTemplateElement).content.cloneNode(true));
    }
    ...
    const playerBtn = document.getElementById('playerBtn') as HTMLButtonElement;
    ...
    playerBtn.addEventListener('click', function() {
        ...
        if(!isPlaying) {
            ...
            (document.getElementById('status') as HTMLElement).innerHTML = 'Playing';
            isPlaying=true;
            ...
            (document.getElementById('status') as HTMLElement).innerHTML = 'Paused';
...
function loadfile(input: any) {
    ...
    reader.onloadend = function() {
        const length = parseInt(
            (reader.result as string).substr(0, (reader.result as string).indexOf("\n")));
        const json_plus_audio = (reader.result as string).substr((reader.result as string).indexOf("\n")+1);
...
function rgbaToHex (rgba: string) {
    ...
    let r: number = parseInt((inParts[0].substring(1)).trim(), 10);
    let g: number = parseInt((inParts[1]).trim(), 10);
    let b: number = parseInt((inParts[2]).trim(), 10);
    let a: number = parseFloat(
        parseFloat((inParts[3].substring(0, inParts[3].length - 1)).trim()).toFixed(2));
    var outParts = [
      r.toString(16),
      g.toString(16),
      b.toString(16),
...
function create_node(node_info: any) { 
    let node: any = null;
...
function destroy_node(node_info: any) {
    let node = stage.findOne(`#${node_info.id}`);
    if(node === null || node === undefined) return;
    node.destroy();
}

function change_node(node_info: any) {
    let node = stage.findOne(`#${node_info.id}`);
    if(node===null || node===undefined) return;
    node.setAttrs(node_info.attrs);
}

function move_node(node_info: any) {
    let node = stage.findOne(`#${node_info.id}`);
    if(node===null || node===undefined) return;
...
function append_to_brush(node_info: any) {
    let node = stage.findOne(`#${node_info.id}`) as Konva.Line;
    if(node===null || node === undefined) return;
...
function fill_node(node_info: any) {
    let node = stage.findOne(`#${node_info.id}`);
    if(node===null || node === undefined) return;
    // console.log(rgbaToHex(node_info.attrs.fill), 'fill');
    (node as Konva.Shape).fill(rgbaToHex(node_info.attrs.fill));
}

function stroke_node(node_info: any) {
    let node = stage.findOne(`#${node_info.id}`);
    if(node===null || node === undefined) return;
    // console.log(rgbaToHex(node_info.attrs.stroke), 'stroke');
    (node as Konva.Shape).stroke(rgbaToHex(node_info.attrs.stroke));
}

function chalkout(handle?: number | undefined) {
    ...
    if(idx === player_array.length) {
        let playerBtn: HTMLButtonElement = document.getElementById("playerBtn") as HTMLButtonElement;
        (document.getElementById('status') as HTMLElement).innerHTML = 'Play complete';
        playerBtn.innerHTML = `<i class="fas fa-play"></i>`;
        idx = 0;
        start_time = 0;
        elapsed_time = 0;
...
function streamer() {
    streamid = (document.getElementById('streamid') as HTMLInputElement).value.trim();
    if(streamid === '') return;
    let peer = new Peer();
    // console.log(peer);
    ...
    peer.on('open', function(id) {
        ...
        conn.on('open', function() {
            idx = 0;
            (document.getElementById('status') as HTMLElement).innerHTML = `Streaming`;
        ...
        // call.on('open', function() {
            ...
            call.on('stream', function(stream) {
                ...
                (document.getElementById('audioPlayer') as HTMLAudioElement).oncanplay = function() {
